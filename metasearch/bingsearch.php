...
<?php if($_SERVER['REQUEST_METHOD']='POST')
	{
		// if the search is submitted from the homepage, carrying out the following instructions:
	//	}
//		
//else{
		if($_POST['SearchType?']=='Web')
   {	// if the user specifies a Web Search, carry out the following instructions:
    echo'<div id="mainwrapper">
   
<div id="individual_or_aggregated_results">';
...
<div id="related_terms_agg_or_individual">
...
echo '<div id="related_search_vertical">';
	...
	foreach ($alternative as $test)
	{
		...
		{
		$test3 = ucwords($test2);
		printf('<a href="bing_related.php?query=%1$s">%1$s</a><br/>', $test3);
...
echo'</div>'; // end of main wrapper
  ...
  if($_POST['SearchType?']=='Images')
   { //if the user specifies an Images search, carry out the following instructions:
    echo'<div id="mainwrapper">';
...
echo'</div>';
   ...
   { //if the user specifies a News search, carry out the following instructions:
	   echo'<div id="mainwrapper">';
