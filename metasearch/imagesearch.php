...
echo '<img src="Resources/Images/background/yahoo.jpg" width="300" height="125" />';
	...
	$cc_key  = "dj0yJmk9VGc4RVplbUFrNkw1JmQ9WVdrOVZYaDRXR0Y1TTJNbWNHbzlNVFl6TmpJMk5qTTJNZy0tJnM9Y29uc3VtZXJzZWNyZXQmeD0zMA--";
		...
		$request2 = OAuthRequest::from_consumer_and_token($consumer2, NULL,"POST", $url2, $args2);
		...
		$request3 = OAuthRequest::from_consumer_and_token($consumer3, NULL,"POST", $url3, $args3);
...
echo'<div id="individual_or_aggregated_results" >';
	...
	$cc_key  = "dj0yJmk9VGc4RVplbUFrNkw1JmQ9WVdrOVZYaDRXR0Y1TTJNbWNHbzlNVFl6TmpJMk5qTTJNZy0tJnM9Y29uc3VtZXJzZWNyZXQmeD0zMA--";
		...
		$yahooarray[$i]['picture'] = $yahooarray[$i]['thumbnailurl'];
		unset($yahooarray[$i]['thumbnailurl']);
		}
...
if ($_POST['query'])
		...
		$googlearray = array_merge_recursive($googlearray1,$googlearray2,$googlearray3,$googlearray4,$googlearray5,$googlearray6,$googlearray7,$googlearray8,$googlearray9,$googlearray10); //for 100 results
			
		for ($i=0;$i<count($googlearray);$i++)
		{
			$googlearray[$i]['picture'] = $googlearray[$i]['image']['thumbnailLink'];
		...
		for ($i=0;$i<count($bingarray);$i++)
		{
		$bingarray[$i]['picture'] = $bingarray[$i]['Thumbnail']['MediaUrl'];
		$bingarray[$i]['title'] = $bingarray[$i]['Title'];
		unset($bingarray[$i]['Title']);
		$bingarray[$i]['link'] = $bingarray[$i]['MediaUrl'];
		unset($bingarray[$i]['MediaUrl']);
		}
...
foreach ($lists as $currentList)
{
    ...
    foreach ($currentList as $entry)
    {
        ...
        else
        {
            ...
            $combinedRank[$entry["link"]]['score'] = 1 / (60 + $currentRank);
			$combinedRank[$entry["link"]]['picture'] = $entry["picture"];
...
foreach ($combinedRank as $finallist)
{
  	...
  	echo "<a href=\"{$finallist['link']}\" target=\"_blank\"><img src= \"{$finallist['picture']}\" height=\"175\" width =\"225\"/></a>"; 
 ...
 <img src="Resources/Images/background/googlebingyahoo.jpg" width="138" height="107" />
   ...
   Related Searches:
	...
	foreach ($alternative as $test)
	{
		...
		{
		$test3 = ucwords($test2);
		printf('<li class="related_search_vertical"><a href="images_related_agg.php?query=%1$s" title="%1$s" id="costslink">%1$s</a></li>', $test3);
