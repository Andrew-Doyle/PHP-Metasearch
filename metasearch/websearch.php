...
echo'<div id="column_1_of_3">';
	...
	//	for ($i=0;$i<count($yahooarray);$i++) //not required for non agg
//		{
...
//		}
  ...
  //echo "<li class=\"titlelink\"> <a href=\"{$yahoo['link']}\"><font color ='blue'>{$yahoo['title']}</font></a></li>"."<li class = \"displaylink\">{$yahoo['link']}</li>"."<li class =\"snippet\">{$yahoo['snippet']}</li>".$newline; //not required for non agg
  
  echo "<li class=\"titlelink\"> <a href=\"{$yahoo['url']}\"><font color ='blue'>{$yahoo['title']}</font></a></li>"."<li class = \"displaylink\">{$yahoo['url']}</li>"."<li class =\"snippet\">{$yahoo['abstract']}</li>".$newline;
		}
...
echo'<div id="column_2_of_3">';
	...
	if ($_POST['query'])
	{
		...
		{
			//$googlearray[$i]['link'] = str_replace("www.","",$googlearray[$i]['link']); //not required for non aggregated results
		}
...
echo'<div id="column_3_of_3">';
	...
	//	for ($i=0;$i<count($bingarray);$i++) //not required for non agg
//		{
...
//		}
		...
		foreach ($bingarray as $bing)
		{
 		...
 		echo "<li class=\"titlelink\"> <a href=\"{$bing['Url']}\"><font color ='blue'>{$bing['Title']}</font></a></li>"."<li class = \"displaylink\">{$bing['Url']}</li>"."<li class =\"snippet\">{$bing['Description']}</li>".$newline;
		}
 ...
 <img src="Resources/Images/background/googlebingyahoo.jpg" width="138" height="107" />
   ...
   Related Searches:
	...
	foreach ($alternative as $test)
	{
		...
		$test3 = ucwords($test2);
		printf('<li><a href="web_agg_related.php?query=%1$s">%1$s</a></li>', $test3);
